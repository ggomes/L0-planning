...
X(1).fwy           = '680N';
X(1).xlsx_file     = fullfile(cfg_folder,'I680NB_Data_nooverride_up.xlsx');
X(1).cntrl_suffix  = 'rm_nooverride_up';

for ii = 1:length(X)
    run_scenario(X(ii).fwy,X(ii).xlsx_file,X(ii).cntrl_suffix)
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function run_scenario(fwy,xlsx_file,cntrl_suffix)
        ...
        is_5min_gp = true; %~isempty(strfind(beatsprop_gp,'beats_gp_5min.properties'));
        ...
        act_and_ctrl = xml_read(fullfile(cfg_folder,'act_cntrl_sr_generator.xml'));
        ...
        for i=1:length(fr_links_ids)
            ...
            for j=1:length(srp.splitratio)
                ...
                sr = comp_sr(ind,5);
                sr = sr(2:end);
%                 sr = mean(reshape(sr(2:end),60,288),1);
        ...
        cfg_srout2 = fullfile(cfg_gen_folder,'srout.xml');
        sr_out2 = fullfile(beats_out_folder,'sr');
        ptr.reset_simulation;
        ...
        act_and_ctrl = xml_read(fullfile(cfg_folder,[cntrl_suffix '.xml']));
        ptr.scenario_ptr.scenario.ControllerSet = act_and_ctrl.ControllerSet;
        ptr.scenario_ptr.scenario.ActuatorSet = act_and_ctrl.ActuatorSet;
        ptr.scenario_ptr.scenario.SensorSet = act_and_ctrl.SensorSet;
                
        % run scenario ....................................................
        cfg_scenario     = fullfile(cfg_folder,[fwy '_' cntrl_suffix '.xml']);
        scenario_out     = fullfile(beats_out_folder,[fwy '_' cntrl_suffix']);
        ptr.reset_simulation;
